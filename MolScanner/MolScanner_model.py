...
from drawing_engine import generate_image_from_smiles, generate_image_from_graph, _blank_image
...
# ======================== Inference Functions ========================
    ...
    def _sample_decode(
        ...
        """Sample a token sequence from the model's distribution.

        Unlike greedy decoding (argmax), this samples each token from
        ``Categorical(softmax(logits / temperature))`` and accumulates
        the log-probability of the sampled sequence for REINFORCE.

        Args:
            feat: [1, d_model, H', W'] image features (single image).
            max_len: maximum decoding length.
            device: torch device.
            temperature: softmax temperature (>1 = more random, <1 = sharper).

        Returns:
            seq: sampled token sequence (list of ints, includes SOS and EOS).
            seq_log_prob: sum of log-probabilities of all sampled tokens (float).
        """
        seq = [self.vocab.sos_idx]
        seq_log_prob = 0.0
        ...
        for _ in range(max_len):
            tgt = torch.tensor([seq], dtype=torch.long, device=device)
            logits = self.predict_step(feat, tgt)[0]  # [vocab_size]
            ...
            seq_log_prob += dist.log_prob(token).item()
            seq.append(token.item())

            if token.item() == self.vocab.eos_idx:
                break

        return seq, seq_log_prob
...
class CycleConsistencyLoss(nn.Module):
    """Frozen pretrained MolDiscriminator for cycle-consistency image loss.

    Wraps a pretrained SiameseNetwork and provides:
      - Image preprocessing for reconstructed images
      - Embedding-space distance computation (original vs reconstructed)
      - Fully frozen — no parameters are trained
    """
    ...
    def __init__(
        ...
        super().__init__()
        from MolDiscriminator_model import SiameseNetwork

        self.disc = SiameseNetwork(embedding_dim=embedding_dim, pretrained=False)
        state_dict = torch.load(discriminator_path, map_location='cpu', weights_only=True)
        ...
        # Running EMA baseline for REINFORCE (avoids zero-advantage with n_valid=1)
        self._running_baseline: Optional[float] = None
        self._ema_alpha = 0.1  # EMA update rate

        # Preprocessing for reconstructed images (matches discriminator training)
        self._pad_to_square = PadToSquare(fill=255)
        self._disc_transforms = A.Compose([
    ...
    def preprocess_image(self, img_np: np.ndarray) -> torch.Tensor:
        """Preprocess a raw numpy image (HWC, uint8, RGB) for the discriminator."""
        img, _ = self._pad_to_square(img_np)
        return self._disc_transforms(image=img)['image']  # [3, H, W]

    @torch.no_grad()
    def compute_distances(
        self, imgs1: torch.Tensor, imgs2: torch.Tensor
    ) -> torch.Tensor:
        """Compute pairwise L2 embedding distances.

        Args:
            imgs1: [N, 3, H, W] preprocessed images (e.g. originals).
            imgs2: [N, 3, H, W] preprocessed images (e.g. reconstructed).
        Returns:
            distances: [N] L2 distances in embedding space.
        """
        emb1 = self.disc.forward_once(imgs1)
        emb2 = self.disc.forward_once(imgs2)
...
def compute_image_loss(
    ...
    device: torch.device,
    max_samples: int = 4,
    max_decode_len: int = 300,
    num_samples_per_image: int = 2,
    temperature: float = 1.0,
) -> Tuple[torch.Tensor, Dict]:
    """Compute cycle-consistency image loss via REINFORCE with sampling.

    Pipeline
    --------
    1. For each of ``max_samples`` images, sample ``num_samples_per_image``
       token sequences from the model's distribution (temperature sampling).
       Log-probabilities are collected **during** sampling — no separate
       forward pass is needed.
    2. Decode tokens → graphs (symbols, binned coords, edges).
    3. Render predicted graphs → reconstructed molecule images (CPU).
    4. Compare original vs reconstructed via frozen MolDiscriminator.
    5. Compute per-image advantage-normalized REINFORCE gradient.

    Advantage computation (per image *k* with K samples):
        When K ≥ 2:  advantage_{k,j} = (r_{k,j} − mean_j(r_{k,·})) / (std_j(r_{k,·}) + ε)
        When K = 1:  advantage_{k,1} = (r_{k,1} − EMA_baseline) / σ_running

    This per-image normalization is the standard REINFORCE baseline: compare
    different predictions for the **same input**, which gives lower-variance
    gradients than comparing across different inputs.

    Args:
        model: MolScanner (DDP-wrapped or raw).
        images: [B, 3, H, W] current training batch (normalized tensors).
        cycle_loss: frozen discriminator module.
        vocab: MolScannerVocab.
        device: torch device.
        max_samples: max images to evaluate per call.
        max_decode_len: maximum autoregressive decoding length.
        num_samples_per_image: K — number of independent samples per image.
        temperature: softmax temperature for sampling (1.0 = unscaled).

    Returns:
        loss: scalar tensor with gradient (REINFORCE).
        stats: dict with monitoring statistics.

    Invalid-sample penalty
    ----------------------
    Samples that fail decode/render (invalid SMILES, RDKit error, Indigo
    crash, blank image) are NOT discarded.  They receive a fixed penalty
    distance of 2.0 (the theoretical maximum L2 distance for unit-norm
    embeddings), i.e. reward = −2.0.  This ensures the model gets a
    gradient signal to **avoid** generating unparseable sequences.
    """
    actual_model = model.module if hasattr(model, 'module') else model
    B = images.size(0)
    n_images = min(max_samples, B)
    K = num_samples_per_image

    stats: Dict = {
        'n_total': n_images, 'n_valid': 0, 'n_samples': 0,
        'avg_distance': 0.0, 'valid_ratio': 0.0,
    }

    # === Phase 1: Sample K sequences per image ===
    was_training = actual_model.training
    actual_model.eval()

    try:
        with torch.no_grad():
            img_features = actual_model.image_encoder(images[:n_images])
    ...
    # sample_data[img_idx] = list of (seq, seq_log_prob, result_dict)
    sample_data: List[List[Tuple[List[int], float, Dict]]] = [[] for _ in range(n_images)]

    try:
        with torch.no_grad():
            for img_idx in range(n_images):
                feat = img_features[img_idx:img_idx + 1]
                for _ in range(K):
                    seq, seq_lp = actual_model._sample_decode(
                        feat, max_decode_len, device, temperature,
                    )
                    # Decode sequence → graph + bond predictions
                    result = actual_model._postprocess_sequence(seq, feat, device)
                    sample_data[img_idx].append((seq, seq_lp, result))
    except Exception:
        if was_training:
            actual_model.train()
        return torch.tensor(0.0, device=device, requires_grad=True), stats
    ...
    # === Phase 2: Render & compute discriminator rewards ===
    # Separate flat lists for valid and invalid samples.
    # Invalid samples (failed decode / render) receive a penalty reward so that
    # the model learns to avoid generating unparseable sequences.
    INVALID_PENALTY_DISTANCE = 2.0  # max L2 distance for unit-norm embeddings

    # --- Valid samples (successfully rendered) ---
    valid_img_indices: List[int] = []
    valid_seqs: List[List[int]] = []
    valid_seq_lps: List[float] = []
    valid_recon_tensors: List[torch.Tensor] = []

    # --- Invalid samples (failed decode / render) ---
    invalid_img_indices: List[int] = []
    invalid_seqs: List[List[int]] = []
    invalid_seq_lps: List[float] = []

    for img_idx, samples in enumerate(sample_data):
        for seq, seq_lp, result in samples:
            # Check if decode succeeded
            if not result.get('success') or not result.get('symbols'):
                invalid_img_indices.append(img_idx)
                invalid_seqs.append(seq)
                invalid_seq_lps.append(seq_lp)
                continue

            graph = {
                'symbols': result['symbols'],
                'coords': result['coords'],
                'edges': result['bond_mat'],
            }

            try:
                img_recon, _, _, ok = generate_image_from_graph(
                    graph, default_drawing_style=True,
                )
            except (FunctionTimedOut, Exception):
                invalid_img_indices.append(img_idx)
                invalid_seqs.append(seq)
            ...
            if not ok or img_recon is None or img_recon.size == 0:
                invalid_img_indices.append(img_idx)
                invalid_seqs.append(seq)
                invalid_seq_lps.append(seq_lp)
                continue
            ...
            # Convert to RGB
            if img_recon.ndim == 2:
                img_recon = cv2.cvtColor(img_recon, cv2.COLOR_GRAY2RGB)
            elif img_recon.shape[2] == 4:
                img_recon = img_recon[:, :, :3]
            if img_recon.shape[2] == 3:
                img_recon = cv2.cvtColor(img_recon, cv2.COLOR_BGR2RGB)

            recon_tensor = cycle_loss.preprocess_image(img_recon)
            valid_recon_tensors.append(recon_tensor)
            valid_img_indices.append(img_idx)
            valid_seqs.append(seq)
            valid_seq_lps.append(seq_lp)

    n_valid = len(valid_img_indices)
    n_invalid = len(invalid_img_indices)
    n_total_samples = n_valid + n_invalid
    n_valid_images = len(set(valid_img_indices))

    stats['n_valid'] = n_valid_images
    stats['n_samples'] = n_total_samples
    stats['n_invalid'] = n_invalid
    stats['valid_ratio'] = n_valid_images / max(n_images, 1)

    if n_total_samples == 0:
        return torch.tensor(0.0, device=device, requires_grad=True), stats

    # Compute discriminator distances for valid samples
    if n_valid > 0:
        recon_batch = torch.stack(valid_recon_tensors).to(device)
        orig_batch = torch.stack([images[i] for i in valid_img_indices]).to(device)
        valid_distances = cycle_loss.compute_distances(orig_batch, recon_batch)  # [n_valid]
        stats['avg_distance'] = valid_distances.mean().item()
        stats['min_distance'] = valid_distances.min().item()
        stats['max_distance'] = valid_distances.max().item()
    else:
        valid_distances = torch.tensor([], device=device)
        stats['avg_distance'] = INVALID_PENALTY_DISTANCE
        stats['min_distance'] = INVALID_PENALTY_DISTANCE
        stats['max_distance'] = INVALID_PENALTY_DISTANCE

    # Merge valid + invalid into combined flat lists
    # Order: [valid_0, valid_1, ..., invalid_0, invalid_1, ...]
    all_img_indices = valid_img_indices + invalid_img_indices
    all_seqs = valid_seqs + invalid_seqs

    # Build combined reward vector: valid get -distance, invalid get -PENALTY
    all_rewards = torch.cat([
        -valid_distances.detach(),
        torch.full((n_invalid,), -INVALID_PENALTY_DISTANCE, device=device),
    ]) if n_valid > 0 else torch.full(
        (n_invalid,), -INVALID_PENALTY_DISTANCE, device=device,
    )

    stats['invalid_ratio'] = n_invalid / max(n_total_samples, 1)

    # === Phase 3: Per-image advantage normalization ===
    advantages = torch.zeros_like(all_rewards)

    img_idx_tensor = torch.tensor(all_img_indices, device=device)
    unique_imgs = img_idx_tensor.unique()

    per_image_advantage_used = False
    for uid in unique_imgs:
        mask = (img_idx_tensor == uid)
        img_rewards = all_rewards[mask]

        if img_rewards.numel() >= 2:
            # Per-image baseline: mean of K samples for the SAME image
            mu = img_rewards.mean()
            sigma = img_rewards.std()
            if sigma > 1e-8:
                advantages[mask] = (img_rewards - mu) / sigma
            else:
                advantages[mask] = img_rewards - mu
            per_image_advantage_used = True
        else:
            # Only 1 sample for this image → use EMA baseline
            r = img_rewards[0].item()
            if cycle_loss._running_baseline is None:
                cycle_loss._running_baseline = r
            baseline = cycle_loss._running_baseline
            advantages[mask] = img_rewards - baseline

    # Update EMA baseline with batch mean of VALID rewards only
    # (including invalid penalty in baseline would skew it)
    if n_valid > 0:
        batch_mean = (-valid_distances).mean().item()
    else:
        batch_mean = -INVALID_PENALTY_DISTANCE
    if cycle_loss._running_baseline is None:
        cycle_loss._running_baseline = batch_mean
    alpha = cycle_loss._ema_alpha
    cycle_loss._running_baseline = (1 - alpha) * cycle_loss._running_baseline + alpha * batch_mean

    # === Phase 4: REINFORCE loss ===
    # Re-compute log-probs for ALL sampled sequences (valid + invalid) WITH gradient.
    max_seq_len = max(len(s) - 1 for s in all_seqs)
    if max_seq_len <= 0:
        return torch.tensor(0.0, device=device, requires_grad=True), stats

    padded_input = torch.full(
        (n_total_samples, max_seq_len), vocab.pad_idx, dtype=torch.long, device=device,
    )
    padded_target = torch.full(
        (n_total_samples, max_seq_len), vocab.pad_idx, dtype=torch.long, device=device,
    )
    target_mask = torch.zeros(n_total_samples, max_seq_len, dtype=torch.float, device=device)

    for idx, seq in enumerate(all_seqs):
        seq_len = min(len(seq) - 1, max_seq_len)
        padded_input[idx, :seq_len] = torch.tensor(seq[:seq_len], dtype=torch.long)
        padded_target[idx, :seq_len] = torch.tensor(seq[1:seq_len + 1], dtype=torch.long)
        target_mask[idx, :seq_len] = 1.0

    padding_mask = (padded_input == vocab.pad_idx)

    # Gather images for each sample (valid + invalid)
    img_subset = torch.stack([images[i] for i in all_img_indices]).to(device)

    # Forward pass WITH gradient (through model.module to avoid DDP conflicts)
    img_feat = actual_model.image_encoder(img_subset)
    img_feat = actual_model.pos_enc_2d(img_feat)
    _, logits = actual_model.sequence_decoder(
        img_feat, padded_input, tgt_key_padding_mask=padding_mask,
    )

    log_probs = F.log_softmax(logits, dim=-1)
    token_log_probs = log_probs.gather(2, padded_target.unsqueeze(-1)).squeeze(-1)
    seq_log_probs = (token_log_probs * target_mask).sum(dim=-1)  # [n_total_samples]

    # REINFORCE: loss = −advantage · log p(ŷ|x)
    reinforce_loss = -(advantages * seq_log_probs).mean()

    # Monitoring stats
    stats['running_baseline'] = cycle_loss._running_baseline if cycle_loss._running_baseline is not None else 0.0
    stats['avg_advantage'] = advantages.mean().item()
    stats['per_image_advantage'] = per_image_advantage_used

    return reinforce_loss, stats
...
def _inference_worker(rank: int, world_size: int, model: MolScannerModel,
...
def predict_multigpu(model: MolScannerModel, image_paths: List[str],
...
def train(
        ...
        image_loss_every_n_steps: int = 50,
        image_loss_max_samples: int = 4,
        image_loss_max_decode_len: int = 300,
        image_loss_num_samples_per_image: int = 2,
        image_loss_temperature: float = 1.0,
        image_loss_start_epoch: int = 0,
) -> List[float]:
    """
    DDP training loop with cosine-annealed LR, AMP, and per-epoch validation.
    
    Supports an optional **cycle-consistency image loss** via REINFORCE:
      1. Sample K sequences per image from the model's distribution
      2. Render predicted graphs → reconstructed molecule images
      3. Compare originals vs reconstructed via a frozen MolDiscriminator
      4. Per-image advantage normalization for low-variance REINFORCE gradient

    Launch with torchrun:
        torchrun --nproc_per_node=<N_GPUS> train_MolScanner_cycle_consistency.py
    ...
    Args:
        ...
        discriminator_path: Path to pretrained MolDiscriminator checkpoint (.pth).
            If None or not found, image loss is disabled.
        image_loss_weight: Weight λ for image loss (total = supervised + λ·image).
        image_loss_every_n_steps: Compute image loss every N training steps.
        image_loss_max_samples: Max images to evaluate per call.
        image_loss_max_decode_len: Max autoregressive decoding length for image loss.
        image_loss_num_samples_per_image: K — number of samples per image for REINFORCE.
        image_loss_temperature: Softmax temperature for sampling (1.0 = unscaled).
        image_loss_start_epoch: Delay image loss until this epoch (1-indexed).
            0 = enabled from the start. Recommended: 2–3 (let supervised loss
            stabilize first so sampled sequences are more often valid).
        resume_from: Path to a pretrained MolScanner checkpoint (.pth) to load
            before training. Useful for:
              - RL fine-tuning a pretrained model (set image_loss_start_epoch=1)
              - Resuming interrupted training
            When set, ``pretrained`` (ImageNet weights) is automatically ignored
            since the checkpoint already contains trained weights.
    ...
    if discriminator_path is not None and os.path.exists(discriminator_path):
        cycle_loss_module = CycleConsistencyLoss(
            discriminator_path=discriminator_path,
            device=device,
            image_size=image_size,
        )
        if is_main_process():
            print(f'Loaded frozen MolDiscriminator from: {discriminator_path}')
            print(f'Image loss: weight={image_loss_weight}, '
                  f'every {image_loss_every_n_steps} steps, '
                  f'max_samples={image_loss_max_samples}, '
                  f'max_decode_len={image_loss_max_decode_len}, '
                  f'start_epoch={image_loss_start_epoch}')
    elif discriminator_path is not None:
        if is_main_process():
            print(f'WARNING: discriminator_path not found: {discriminator_path}. '
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for _step in range(steps_in_epoch):
            ...
            img_stats = {}  # Reset per-step; populated when image loss runs
            ...
            if scaler is not None:
                ...
                # Supervised backward (gradient accumulation start)
                scaler.scale(losses['total_loss']).backward()
                
                # --- Cycle-consistency image loss (REINFORCE, every N steps) ---
                if (cycle_loss_module is not None
                        and epoch >= image_loss_start_epoch
                        and global_step % image_loss_every_n_steps == 0
                        and global_step > 0):
                    try:
                        with torch.amp.autocast('cuda'):
                            img_loss, img_stats = compute_image_loss(
                                model=model,
                                images=images,
                                cycle_loss=cycle_loss_module,
                                vocab=vocab,
                                device=device,
                                max_samples=image_loss_max_samples,
                                max_decode_len=image_loss_max_decode_len,
                                num_samples_per_image=image_loss_num_samples_per_image,
                                temperature=image_loss_temperature,
                            )
                        if img_loss.requires_grad:
                            scaler.scale(image_loss_weight * img_loss).backward()
                            did_backward = True
                        else:
                            did_backward = False
                        losses['image_loss'] = img_loss
                        if is_main_process():
                            weighted_loss = (image_loss_weight * img_loss.item()) if img_loss.requires_grad else 0.0
                            print(f'\n  [Image Loss] step={global_step}, '
                                  f'loss={img_loss.item():.4f} (×{image_loss_weight}={weighted_loss:.4f}), '
                                  f'valid={img_stats.get("n_valid", 0)}/{img_stats.get("n_total", 0)} img, '
                                  f'samples={img_stats.get("n_samples", 0)} '
                                  f'(invalid={img_stats.get("n_invalid", 0)}), '
                                  f'avg_dist={img_stats.get("avg_distance", 0):.4f}, '
                                  f'backward={did_backward}')
                    except Exception as e:
                        if is_main_process():
                            print(f'\n[!] Image loss failed at step {global_step}: {e}')
                ...
                if (cycle_loss_module is not None
                    ...
                    try:
                        ...
                        if img_loss.requires_grad:
                            (image_loss_weight * img_loss).backward()
                            did_backward = True
                        else:
                            did_backward = False
                        losses['image_loss'] = img_loss
            ...
            if is_main_process():
                ...
                if img_stats and img_stats.get('n_valid', 0) > 0:
                    postfix['dist'] = f"{img_stats['avg_distance']:.3f}"
                    postfix['valid'] = f"{img_stats['n_valid']}/{img_stats['n_total']}"
                ...
                # Cycle-consistency metrics (meaningful quantities, not REINFORCE loss)
                if img_stats and img_stats.get('n_samples', 0) > 0:
                    writer.add_scalar('CycleLoss/avg_distance', img_stats['avg_distance'], global_step)
                    writer.add_scalar('CycleLoss/min_distance', img_stats.get('min_distance', 0), global_step)
                    writer.add_scalar('CycleLoss/max_distance', img_stats.get('max_distance', 0), global_step)
                    writer.add_scalar('CycleLoss/invalid_ratio', img_stats.get('invalid_ratio', 0), global_step)
